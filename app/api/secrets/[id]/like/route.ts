  ...
  try {
    ...
    if (!rateLimit.allowed) {
      return NextResponse.json(
        { 
          error: `Rate limit exceeded. You can like ${RATE_LIMITS.LIKE.count} secrets per hour.`,
          remainingCount: 0
        },
        { status: 429 }
      )
    ...
    // 增加点赞数
    const { error } = await supabase.rpc('increment_likes', { secret_id: id })

    if (error) {
      console.error('Like secret error:', error)
      return NextResponse.json({ error: 'Failed to like secret' }, { status: 500 })
    }
    ...
    console.error('POST /api/secrets/[id]/like error:', error)
